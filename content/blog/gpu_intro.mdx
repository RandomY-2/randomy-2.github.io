...
The CUDA programming model exposes the GPU’s parallel execution capabilities to the developer. A CUDA program
consists of both
**host code**, which runs on the CPU, and **device code**, which runs on the GPU. The CPU (host) initiates
GPU computation by launching **kernel functions**—CUDA device-side functions marked with the `__global__`
qualifier—that are executed by many parallel threads on the GPU.

### Host–Device Interaction

1. **Initialization** The host allocates buffers in host and device memory and, if necessary, pins host pages to enable
direct-memory-access (DMA) transfers.
2. **Data Movement** Input data are transferred to global memory over PCIe or NVLink.  Asynchronous copies (`cudaMemcpyAsync`)
let the host queue transfers and immediately proceed.
3. **Kernel Launch** With the execution configuration set, the host enqueues the kernel.  The GPU driver partitions the
grid across available SMs; individual blocks are dispatched as soon as hardware resources (registers, shared memory, warp
slots) are free.
4. **Execution & Concurrency** Within each SM, a warp scheduler issues instructions from resident warps every cycle,
hiding global-memory latency by readily switching to another ready warp.
5. **Completion & Copy-Back** When all blocks finish, control returns to the host.  Output data are copied back to host
memory, after which the host proceeds with post-processing or launches subsequent kernels.
...
earlier.  Warp divergence, caused by per-thread control-flow variation, is handled through per-thread predication
